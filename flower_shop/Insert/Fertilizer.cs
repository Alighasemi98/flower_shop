...
namespace flower_shop.Insert
{
    public partial class Fertilizer : Form
    {
        ...
        private void but_addflower_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    se.Open();
                    SqlCommand comm = new SqlCommand("INSERT INTO TFertilizer(Name_Fertilizer,Buy_Price,Sale_Price,Number,Explanation,Picture,time) VALUES (@n,@b,@s,@u,@d,@p,@t)", se);
        ...
        private void but_adddfertilizer_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string name = comfertilizer.SelectedItem.ToString();

                    SqlConnection se = new SqlConnection("Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Users\\amin\\source\\repos\\flower_shop\\flower_shop\\DatabaseFlowershop.mdf;Integrated Security=True");
                    se.Open();

                    string queryy = "SELECT Number FROM TFertilizer WHERE Name_Fertilizer='" + name + "'";
                    ...
                    string query = "UPDATE TFertilizer SET Buy_Price='" + buy + "',Sale_Price='" + sale + "',Number='" + xz + "' WHERE Name_Fertilizer='" + name + "'";
                    ...
                    ClearTxt(this);
                    DGV();
                    comfertilizer.SelectedItem = null;
                    comitem();
                }
            }
            catch
        ...
        private void comfertilizer_SelectedIndexChanged(object sender, EventArgs e)
        {
            comitemm();
        }
        public void DGV()
        {
            ...
            {
                SqlConnection se = new SqlConnection("Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Users\\amin\\source\\repos\\flower_shop\\flower_shop\\DatabaseFlowershop.mdf;Integrated Security=True");
                SqlDataAdapter da = new SqlDataAdapter("SELECT Id, Name_Fertilizer, Buy_Price, Sale_Price, Explanation, Number, time, Picture FROM TFertilizer ", se);
        ...
        private void Fertilizer_Load(object sender, EventArgs e)
        {
            SqlConnection se = new SqlConnection("Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Users\\amin\\source\\repos\\flower_shop\\flower_shop\\DatabaseFlowershop.mdf;Integrated Security=True");
            se.Open();
            string query = "SELECT Name_Fertilizer FROM TFertilizer";
            SqlCommand comm = new SqlCommand(query, se);
            SqlDataReader reader = comm.ExecuteReader();
            while (reader.Read())
            {
                string name = reader["Name_Fertilizer"].ToString();
                comfertilizer.Items.Add(name);
            }
            se.Close();
            DGV();
        }
