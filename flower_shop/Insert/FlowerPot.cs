...
{
    public partial class FlowerPot : Form
    {
        public FlowerPot()
        {
            InitializeComponent();
        ...
        private void but_addflower_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    se.Open();
                    SqlCommand comm = new SqlCommand("INSERT INTO TflowerPot(Name_FlowerPot,Buy_Price,Sale_Price,Number,Explanation,Picture,time) VALUES (@n,@b,@s,@u,@d,@p,@t)", se);
        ...
        private void but_adddflowerpot_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string name = comflowerpot.SelectedItem.ToString();
                    SqlConnection se = new SqlConnection("Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Users\\amin\\source\\repos\\flower_shop\\flower_shop\\DatabaseFlowershop.mdf;Integrated Security=True");
                    se.Open();
                    string queryy = "SELECT Number FROM TFlowerPot WHERE Name_FlowerPot='" + name + "'";
                    ...
                    string query = "UPDATE TFlowerPot SET Buy_Price='" + buy + "',Sale_Price='" + sale + "',Number='" + xz + "' WHERE Name_FlowerPot='" + name + "'";
                    SqlCommand comm = new SqlCommand(query, se);

                    /* comm.Parameters.AddWithValue("@n", name);
                       comm.Parameters.AddWithValue("@b", buy);
                       comm.Parameters.AddWithValue("@s", sale);
                       comm.Parameters.AddWithValue("@u", number);*/
                    ...
                    ClearTxt(this);
                    DGV();
                    comflowerpot.SelectedItem = null;
                    comitem();
                }
            }
            catch
        ...
        private void comflowerpot_SelectedIndexChanged(object sender, EventArgs e)
        {
            comitemm();
        }
        public void DGV()
        {
            ...
            {
                SqlConnection se = new SqlConnection("Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Users\\amin\\source\\repos\\flower_shop\\flower_shop\\DatabaseFlowershop.mdf;Integrated Security=True");
                SqlDataAdapter da = new SqlDataAdapter("SELECT Id, Name_FlowerPot, Buy_Price, Sale_Price, Explanation, Number, time, Picture FROM TFlowerPot ", se);
        ...
        private void FlowerPot_Load(object sender, EventArgs e)
        {
            SqlConnection se = new SqlConnection("Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Users\\amin\\source\\repos\\flower_shop\\flower_shop\\DatabaseFlowershop.mdf;Integrated Security=True");
            se.Open();
            DGV();
            string query = "SELECT Name_FlowerPot FROM TFlowerPot";
            SqlCommand comm = new SqlCommand(query, se);
            SqlDataReader reader = comm.ExecuteReader();
            while (reader.Read())
            {
                string name = reader["Name_FlowerPot"].ToString();
                comflowerpot.Items.Add(name);
            }
